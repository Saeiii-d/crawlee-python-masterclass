...
async def default_handler(context: PlaywrightCrawlingContext) -> None:
    ...
    # Apply a realistic browser-like User-Agent to subsequent page requests.
    ua = (
    ...
    if input_element:
        ...
        logger.info(
            f"Search results loaded for <<{query}>>, "
            f"enqueuing category page..."
        )
...
async def category_handler(context: PlaywrightCrawlingContext) -> None:
    ...
    current_count = len(
        await page.query_selector_all("a[href*='/p/']")
    )
    ...
    try:
        while current_page <= scroll_attempt_cnt:
            ...
            # Use Playwright's scroll capability to trigger JS-loading events
            ...
            if new_count > current_count:
                ...
                logger.info(
                    "No new products loaded after scrolling."
                )
                break
...
async def product_handler(context: PlaywrightCrawlingContext) -> None:
    ...
    if sku_match:
        ...
        logger.warning(
            f"SKU not found in URL: {context.request.url}"
        )

        url_hash = hashlib.md5(
            context.request.url.encode()
        ).hexdigest()
    ...
    title = await page.query_selector(
        "div[class*='Showcase_name'] h1"
    )
    title_text = (
        await title.text_content()
        if title
        else "No title found"
    )
    ...
    price_elements = await page.query_selector_all(
        "div[class*='Showcase_cheapest_seller'] "
        "div[class*='Showcase_buy_box_text']"
    )
    ...
    if price_elements:
        ...
        # Determine availability from the returned buy-box texts instead of
        # assuming that a specific list index contains the status.
        if any("ناموجود" in text for text in price_texts):
            is_available = False

        # Find the seller by its semantic prefix rather than a fixed index.
        for text in price_texts:
            if "مشاهده در" in text or "خرید از" in text:
                cheapest_seller = (
                    text
                    .replace("مشاهده در", "")
                    .replace("خرید از", "")
        ...
        # Find a numeric price from the remaining text elements.
        # This removes the old price_elements[1] assumption and therefore
        # avoids IndexError when only one matching element is present.
        if is_available:
            for text in price_texts:
                if "مشاهده در" in text or "خرید از" in text:
                    continue

                numeric_value = clean_number(text)
    ...
    # Used status
    is_used = False
    used_element = await page.query_selector(
        "div[Showcase_main_info] div[class*='stock-status']"
    )
    ...
    image_elements = await page.query_selector_all(
        "img[class*='ImageVideoSliderGallery_media']"
    )
    ...
    if not image_urls:
        image_elements = await page.query_selector_all(
            "div[class*='imageGallery_cell__'] picture source"
        )
